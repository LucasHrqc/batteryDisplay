  ...
  methods: {
    initBattery() {
      navigator.getBattery().then((batt) => {
        ...
        batt.addEventListener("chargingchange", (batt) => {
          this.updateBattery(batt);
        });
        batt.addEventListener("levelchange", (batt) => {
          this.updateBattery(batt);
        });
      });
    },
    updateBattery(batt) {
      ...
      if (level == 100) {
        ...
        batteryStatusEl.innerHTML =
          'Low battery <i class="ri-plug-line animated-red animated-red"></i>';
      ...
      if (level <= 20) {
        ...
        batteryLiquidEl.classList.remove(
